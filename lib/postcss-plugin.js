 ...
 */
function addFontDeclaration (fontName, postCssRoot, useCssModules, enforcedSvgHeight, svgPaths, descent) {
  // The options are passed as a query string so we use the relative svg paths to reduce the path length per file
  const options = { svgs: svgPaths.relative, name: fontName, enforcedSvgHeight: enforcedSvgHeight, descent };
...
module.exports = postcss.plugin('iconfont-webpack', config => function (root, result) {
  ...
  return getSvgPaths(root, config.resolve, context)
    .then(function (svgPaths) {
      ...
      return Promise.all([
        // add the font faces
        addFontDeclaration(fontName, root, config.modules, config.enforcedSvgHeight, svgPaths, config.descent),
